...
class Observable {
	...
	removeListener(listenerFn) {
		this._listeners.splice(this._listeners.indexOf(listenerFn), 1);
	}

	notifyListeners(propName) {
		this._listeners.forEach(fn => fn(this, propName));
		this._dependentModels.forEach(model => model.notifyListeners());
	}
...
class Store extends Observable {
	...
	initialize() {
		return this.onInitialize().then(() => (this.isReady = true));
	}
}

const createProvider = (ObservableClass, { dependencies = [], name } = {}) => {
	...
	const defaultFactory = args => new ObservableClass(...args);
	const StoreProvider = ({
		children,
		...
		const restSpread = Object.keys(rest).reduce((arr, k) => {
			arr.push(k);
			arr.push(rest[k]);
			return arr;
		}, []);
		...
		useEffect(
			() => {
				...
				if (deps.every(x => x && (x.isReady || !x.initialize))) {
...
const useObservable = (
	...
	useEffect(() => {
		if (observable) {
			return observable.listen((_, propName) => {
				if (
					!dependantProperties ||
					!propName ||
...
const observeScoped = (
	...
	const ScopedComponent = props => {
		const [observable, version] = useScopedObservable(ObservableClass, {
			includeVersion: true,
			dependantProperties,
		});
		const values = {
		...
		return <Component {...props} {...values} $$version={version} />;
	};

	return ScopedComponent;
};

const observe = (Component, propName, dependantProperties) => {
	const ObservingComponent = props => {
		const [observable, version] = useObservable(props[propName], {
			includeVersion: true,
			dependantProperties,
		});
